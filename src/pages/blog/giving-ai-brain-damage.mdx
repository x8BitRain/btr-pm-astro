...
So these corruption values work, this time I'll try to add 9 bytes to every 500 bytes instead of just 2 bytes for every 500 bytes.
...
I increased the amount from adding 9 bytes to every 500 byte to 30. This results in incredibly blown out photos that looks like every color option in Photoshop set to 1000.
...
Shifting the bytes around again to every 400 bytes, it now generates these interesting oil paint style images that are colored vaguely like a city.
